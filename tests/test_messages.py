...
class TestGetMessages:
    ...
    def test_get_all_messages(self, mock_backup: Backup):
        """Test retrieving all messages without filters."""
        df = get_messages(mock_backup)
        assert not df.empty
        assert len(df) == 6  # 6 messages in fixture
    ...
    def test_read_at_timestamp(self, mock_backup: Backup):
    ...
    def test_is_from_me_boolean(self, mock_backup: Backup):
    ...
    def test_reactions_parsed(self, mock_backup: Backup):
    ...
    def test_group_chat_detection(self, mock_backup: Backup):
    ...
    def test_contact_name_me_for_sent(self, mock_backup: Backup):
        """Test that is_from_me messages have contact_name='Me'."""
        df = get_messages(mock_backup)
        sent = df[df["is_from_me"] == True]
        assert not sent.empty
        assert (sent["contact_name"] == "Me").all()

    def test_contact_name_for_received(self, mock_backup: Backup):
        """Test that received messages get contact_name from handle table."""
        df = get_messages(mock_backup)
        received = df[df["is_from_me"] == False]
        assert not received.empty
        # All received messages should have a non-None contact_name
        assert received["contact_name"].notna().all()

    def test_filter_by_phone_number(self, mock_backup: Backup):
        """Test filtering messages by phone number."""
        df = get_messages(mock_backup, phone_numbers="+12345678900")
        assert not df.empty
        # Should return messages between user and this contact only:
        # sent messages (is_from_me) + received from this number.
        # Message 3 (from +19876543210 in group chat) is excluded.
        assert len(df) == 5
        # Should include both sent and received messages
        assert df["is_from_me"].any()
        assert (~df["is_from_me"]).any()

    def test_filter_by_multiple_phones(self, mock_backup: Backup):
    ...
    def test_phone_number_variants(self, mock_backup: Backup):
        """Test that phone number variants work for matching."""
        # Try different formats of same number
        df1 = get_messages(mock_backup, phone_numbers="+12345678900")
        df2 = get_messages(mock_backup, phone_numbers="2345678900")
        df3 = get_messages(mock_backup, phone_numbers="12345678900")
        # All should return same results
        assert len(df1) == len(df2) == len(df3)

    def test_filter_by_date_range(self, mock_backup: Backup):
        """Test filtering messages by date range."""
        # Messages are from 2020-2021 in fixture
        df = get_messages(
            mock_backup, date_range=("2020-01-01", "2020-12-31")
        )
        assert not df.empty
        assert all(df["timestamp"].dt.year == 2020)

    def test_filter_combined(self, mock_backup: Backup):
    ...
    def test_csv_export(self, mock_backup: Backup, tmp_path: Path):
        """Test CSV export functionality."""
        csv_path = tmp_path / "messages.csv"
        df = get_messages(mock_backup, output_csv=csv_path)
        assert csv_path.exists()
        # Read back and verify
        df_read = pd.read_csv(csv_path)
        assert len(df_read) == len(df)

    def test_empty_result(self, mock_backup: Backup):
    ...
    def test_invalid_date_range_format(self, mock_backup: Backup):
        """Test that invalid date_range raises ValueError."""
        with pytest.raises(ValueError, match="tuple"):
            get_messages(mock_backup, date_range="2020-01-01")

    def test_invalid_date_range_order(self, mock_backup: Backup):
        """Test that start > end raises ValueError."""
        with pytest.raises(ValueError, match="must be before"):
            get_messages(
                mock_backup, date_range=("2021-01-01", "2020-01-01")
            )

    def test_attributed_body_fallback(self, mock_backup: Backup):
        """Test that NULL text is filled from attributedBody blob."""
        df = get_messages(mock_backup)
        # Message 6 has NULL text but valid attributedBody
        msg6 = df[df["message_text"] == "Message from attributedBody"]
        assert len(msg6) == 1
